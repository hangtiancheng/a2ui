...
- `catalog.ts` is the port of `A2uiCatalog.render_as_llm_instructions`: it loads the three root spec files, applies
  ...
  It also owns `CATALOG_ID` (the catalog's own `catalogId`, overridden with the shadcn catalog id when `A2UI_MODE=shadcn`).
