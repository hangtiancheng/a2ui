...
The `postinstall` script downloads three official v0.9 spec files to the repository root: `catalog.json` (basic catalog),
`server_to_client.json` and `common_types.json`. The server reads all three at startup and embeds them in the system prompt.
...
## LLM-Direct A2UI Output (`prompt.ts` / `catalog.ts` / `llm.ts`, core design)
...
- `prompt.ts` mirrors the upstream Python `DirectJsonPromptGenerator.generate` section-for-section:
  role description → `## Workflow Description:` (the ported `DEFAULT_WORKFLOW_RULES` plus the tool-calling rules) →
  `## UI Description:` (the ported restaurant_finder `UI_DESCRIPTION`) → the schema block → `### Examples:`.
  `TEXT_SYSTEM_PROMPT` is the port of the sample's `get_text_prompt`.
- `catalog.ts` is the port of `A2uiCatalog.render_as_llm_instructions`: it loads the spec JSON files, applies
  `remove_strict_validation` (drops `additionalProperties`/`unevaluatedProperties: false`), prunes unreferenced `common_types` `$defs`
  (the Common Types section is omitted entirely when nothing remains, as upstream does),
  and renders `---BEGIN A2UI JSON SCHEMA---` … `---END A2UI JSON SCHEMA---` with the server-to-client, common-types and catalog schemas.
  The catalog schema is the official `catalog.json` (18 components), or `packages/shadcn/catalog.json` (65 components)
  when `A2UI_MODE=shadcn`; `CATALOG_ID` is always read from the loaded schema's own `catalogId`.
  Never import `@swifty.js/a2ui-shadcn` here to obtain the catalog: a `Catalog` instance keeps its components in a `Map`,
  so it serializes to `{"components":{}}` and the prompt silently loses the whole component contract (it would also pull React into the server).
- Few-shot examples are dynamically generated by the three builders in `a2ui-messages.ts` (listing / booking form / confirmation page) —
  to change the UI structure, modify the builders; the prompt updates accordingly. Upstream additionally ships a two-column list
  template; this server intentionally has only the single list layout.
...
- **Generated JSON catalog (`catalog.json`)**: `pnpm --filter @swifty.js/a2ui-shadcn build:catalog`
  (`scripts/build-catalog.ts`) converts the 47 extension zod schemas to JSON Schema with `zod-to-json-schema`, keeps the official
  entries for the 18 basic components verbatim, and republishes the whole thing under `SHADCN_CATALOG_ID`.
  The server embeds this file in the system prompt, so **regenerate and commit it whenever catalog schemas change** — a stale file
  silently teaches the LLM the wrong contract. Notes: shared protocol types are re-referenced as
  `common_types.json#/$defs/...` by matching converted shapes (`.describe()` breaks identity), and bare primitives such as
  `ComponentIdSchema` (`z.string()`) are deliberately left inlined, since matching them would relabel every plain string prop;
  the script runs through `tsx --tsconfig tsconfig.app.json` because it needs `jsx: react-jsx` and the `@/*` alias to import the catalog source.
